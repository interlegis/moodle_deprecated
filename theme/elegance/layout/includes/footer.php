...
$hasinterlegis  = (empty($PAGE->theme->settings->interlegis)) ? false : $PAGE->theme->settings->interlegis;
$hasilb  = (empty($PAGE->theme->settings->ilb)) ? false : $PAGE->theme->settings->ilb;
$hasabel  = (empty($PAGE->theme->settings->abel)) ? false : $PAGE->theme->settings->abel;
$hassenado  = (empty($PAGE->theme->settings->senado)) ? false : $PAGE->theme->settings->senado;
$hastcu  = (empty($PAGE->theme->settings->tcu)) ? false : $PAGE->theme->settings->tcu;
$hascamara  = (empty($PAGE->theme->settings->camara)) ? false : $PAGE->theme->settings->camara;
...
$hassocialnetworks = ($hasfacebook || $hastwitter || $hasgoogleplus || $hasflickr || $hasinstagram || $hasvk || $haslinkedin || $haspinterest || $hasskype || $haslinkedin || $haswebsite || $hasinterlegis || $hasilb || $hasabel || $hassenado || $hastcu || $hascamara || $hasyoutube ) ? true : false;
		...
		<div class="col-lg-6 pull-right">
			...
			if ($hassocialnetworks) {
				echo '<ul class="socials unstyled">';
					...
					if ($hasinterlegis) {
					    echo '<a href="'.$hasinterlegis.'" class="socialicon custom interlegis">';
					      echo '<span class="sr-only">'.get_string('socialnetworksicondescriptionwebsite','theme_elegance').'</span>';
					    echo '</a>';
					}

					if ($hasilb) {
					    echo '<a href="'.$hasilb.'" class="socialicon custom ilb">';
					      echo '<span class="sr-only">'.get_string('socialnetworksicondescriptionwebsite','theme_elegance').'</span>';
					    echo '</a>';
					}

					if ($hasabel) {
					    echo '<a href="'.$hasabel.'" class="socialicon custom abel">';
					      echo '<span class="sr-only">'.get_string('socialnetworksicondescriptionwebsite','theme_elegance').'</span>';
					    echo '</a>';
					}

					if ($hastcu) {
                                            echo '<a href="'.$hastcu.'" class="socialicon custom tcu">';
                                              echo '<span class="sr-only">'.get_string('socialnetworksicondescriptionwebsite','theme_elegance').'</span>';
                                            echo '</a>';
                                        }

					if ($hassenado) {
					    echo '<a href="'.$hassenado.'" class="socialicon custom senado">';
					      echo '<span class="sr-only">'.get_string('socialnetworksicondescriptionwebsite','theme_elegance').'</span>';
					    echo '</a>';
					}

					if ($hascamara) {
					    echo '<a href="'.$hascamara.'" class="socialicon custom camara">';
					      echo '<span class="sr-only">'.get_string('socialnetworksicondescriptionwebsite','theme_elegance').'</span>';
					    echo '</a>';
					}
